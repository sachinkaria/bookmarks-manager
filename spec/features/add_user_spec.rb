...
feature 'add user' do
  ...
  scenario 'and user count increases by 1' do
    expect{User.create(name: "Sachin", password: "password", email: "sachin.karia01@gmail.com")}.to change{ User.count }.by(1)
  end
  ...
  scenario 'and the email for that user is correctly stored in our DB' do
    new_user
    expect(User.first.email).to include("sachin.karia01@gmail.com")
  end
